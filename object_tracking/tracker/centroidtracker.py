	...
	def deregister(self, objectID, time):
		...
		if (len(self.car_past_direction[objectID]) > 10): # can safety assume a direction
			...
			if (len(self.road_directions) == 0):
				...
				f.write("time " + str(time) + " " + " x " + str(average_direction[0]) + " y " + str(average_direction[1])+ "\n")
				f.close()

				self.numOfCarExistedLast[" x " + str(average_direction[0]) + " y " + str(average_direction[1])] = 1
				...
				if (found!=True):
					...
					f.write("time " + str(time) + " " + " x " + str(self.road_directions[index][0][0]) + " y " + str(
						self.road_directions[index][0][1]) + "\n")
					f.close()

					_direction = " x " + str(self.road_directions[index][0][0]) + " y " + str(self.road_directions[index][0][1])
					if _direction not in self.numOfCarExistedLast:
						self.numOfCarExistedLast[_direction] = 0

					self.numOfCarExistedLast[_direction] += 1
